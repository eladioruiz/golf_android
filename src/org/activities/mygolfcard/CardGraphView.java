...
public class CardGraphView extends View {
	...
	private boolean connectionOK;	// private String URL_MATCH;
	...
	public CardGraphView(Context context, int match_id, int mitad,
			int type_match) {
		super(context);
	...
	protected void onDraw(Canvas canvas) {
		...
		for (i = FIRST_COL; i < COLS; i++) {
			canvas.drawLine(i * width, 3 * height, i * width, getHeight(),
					light); // lineas vert
			canvas.drawLine(i * width + 1, 3 * height, i * width + 1,
					getHeight(), hilite); // lineas vert
		}

		// Draw the minor horizontal grid lines
		for (i = FIRST_ROW; i < ROWS; i++) {
			canvas.drawLine(0, i * height, getWidth(), i * height, light); // lineas
																			// hoz
		...
		canvas.drawLine(i * width + 1, 3 * height, i * width + 1, getHeight(),
				hilite);
		...
		canvas.drawCircle((float) (2.5 * width), (float) (1.5 * height),
				(float) (1 * width), dark);
		canvas.drawText(mitad == 1 ? "2P" : "1P", (float) (1.75 * width + x),
				(float) (1 * height + y), players);

		canvas.drawText(course_name, (TITLE_COL * width + x),
				(float) (0.75 * height + y), players);
		canvas.drawText(date_hour_match, (TITLE_COL * width + x),
				(float) (1.5 * height + y), players);
		...
		for (i = 5; i <= 13; i++) {
			...
			canvas.drawText("" + holeNumber, (i * width) + x, (j * height) + y,
					nHoles);
		}
		...
		for (i = 5; i <= 13; i++) {
			int holeNumber = i - 4 + ((mitad - 1) * 9);
			canvas.drawText(setInfoHoles(holeNumber), (i * width) + x,
					(j * height) + y, infoHoles);
		}
		...
		for (int iPlayer = 0; iPlayer < auxPlayer.length; iPlayer++) {
			...
			if (auxPlayer[iPlayer] != null) {
				canvas.drawText(
						getPlayerName("" + auxPlayer[iPlayer].getPlayer_id()),
						(i * width) + 1, (j * height) + y, players);
			} else {
				canvas.drawText("---", (i * width) + 1, (j * height) + y,
						players);
			}
			++i;
			...
			for (int z = 1 + ((mitad - 1) * 9); z < 10 + ((mitad - 1) * 9); z++) {
				...
				canvas.drawText("" + arrStrokes[iPlayer][z], ((i + 4) * width)
						+ x, (j * height) + y, strokes);
				++i;
			}
			...
			canvas.drawText("" + sum_mitad, ((i + PARTIAL_COL) * width),
					(j * height) + y, players_total);
			canvas.drawText("" + sum_total, ((i + TOTAL_COL) * width),
					(j * height) + y, players_total);
	...
	private void getInfoCardInternal() {
		...
		try {
			...
			for (int i = 0; i < 4; i++) {
				if (auxPlayer[i] != null) {
					sql = "SELECT * FROM strokes WHERE match_id=" + match_id
							+ " AND player_id=" + auxPlayer[i].getUserWeb_id();
		...
		/*
		 * ERL String sql = ""; int i = 0;
		 * 
		 * sql =
		 * "SELECT course_name, date_hour_match, holes, player1_id, player2_id, player3_id, player4_id FROM matches where id="
		 * + match_id;
		 * 
		 * try { db = ctxCardGraph.openOrCreateDatabase(DATABASE_NAME, 0, null);
		 * 
		 * Cursor c = db.rawQuery(sql, null); colPlayer[0] =
		 * c.getColumnIndex("player1_id"); colPlayer[1] =
		 * c.getColumnIndex("player2_id"); colPlayer[2] =
		 * c.getColumnIndex("player3_id"); colPlayer[3] =
		 * c.getColumnIndex("player4_id");
		 * 
		 * colCourseName = c.getColumnIndex("course_name"); colDateHour =
		 * c.getColumnIndex("date_hour_match"); colnHoles =
		 * c.getColumnIndex("holes");
		 * 
		 * c.moveToLast(); c.moveToFirst(); if (c != null) { i = 0;
		 * 
		 * // recupera los ids de lo jugadores que participan en el partido
		 * aux_player_id[0] = c.getInt(colPlayer[0]); aux_player_id[1] =
		 * c.getInt(colPlayer[1]); aux_player_id[2] = c.getInt(colPlayer[2]);
		 * aux_player_id[3] = c.getInt(colPlayer[3]);
		 * 
		 * // y los datos del partido course_name = c.getString(colCourseName);
		 * date_hour_match = c.getString(colDateHour);
		 * 
		 * for (i=0;i<4;i++) { if (aux_player_id[i]!=0) { sql =
		 * "SELECT * FROM strokes WHERE match_id=" + match_id +
		 * " AND player_id=" + aux_player_id[i];
		 * 
		 * Cursor c2 = db.rawQuery(sql, null); int colHoleNumber =
		 * c2.getColumnIndex("hole"); int colStrokes =
		 * c2.getColumnIndex("strokes");
		 * 
		 * c2.moveToLast(); c2.moveToFirst(); if (c2 != null) { do { int
		 * holeNumber = c2.getInt(colHoleNumber); int strokes =
		 * c2.getInt(colStrokes);
		 * 
		 * arrStrokes[i][holeNumber] = strokes; } while (c2.moveToNext()); }
		 * c2.close(); } } } c.close(); } catch(Exception e) { Log.e("Error",
		 * "Error reading DB", e); } finally { if (db != null) db.close(); }
		 */invalidate();
	}

	private void getInfoCardRemote() {
		// ERL URL_MATCH = ctxCardGraph.getString(R.string.URL_APIS) +
		// ctxCardGraph.getString(R.string.ACTION_MATCH);
		URL_STROKES = ctxCardGraph.getString(R.string.URL_APIS)
				+ ctxCardGraph.getString(R.string.ACTION_STROKES);
	...
	private void setInfoPlayers(String result) {
		...
		try {
			...
			for (int i = 0; i < jsonArr.length(); i++) {
				...
				auxPlayer[i].setPlayer_id(Integer.parseInt(jsonObj
						.getString("id")));
	...
	/*
	 * ERL public String getMatch() { String response;
	 * 
	 * RestClient client = new RestClient(URL_MATCH); client.AddParam("token",
	 * auth_token); client.AddParam("user_id", "" + auth_user_id);
	 * client.AddParam("match_id", "" + match_id);
	 * 
	 * response = ""; try { client.Execute(RequestMethod.POST); response =
	 * client.getResponse(); } catch (Exception e) { e.printStackTrace(); }
	 * 
	 * return response; }
	 */
	...
	/*
	 * ERL public void setInfoMatch(String result) { JSONObject jsonObj;
	 * JSONArray jsonArr; String aux_players;
	 * 
	 * try { jsonObj = new JSONObject(result);
	 * 
	 * aux_players = jsonObj.getString("players"); course_name =
	 * jsonObj.getString("course_name"); date_hour_match =
	 * jsonObj.getString("date_hour_match");
	 * 
	 * jsonArr = new JSONArray(aux_players);
	 * 
	 * for (int i=0; i<jsonArr.length(); i++) { jsonObj = new
	...
	public void setInfoStrokes(int iPlayer, String result) {
		...
		try {
			...
			for (int i = 0; i < jsonArr.length(); i++) {
				...
				hole_number = Integer
						.parseInt(jsonObj.getString("hole_number"));
	...
	private String setInfoHoles(int holeNumber) {
		String res = "--";
	...
	protected class InitTask extends AsyncTask<Context, Integer, String> {
		...
		protected String doInBackground(Context... params) {
			...
			currentMatch = org.classes.mygolfcard.Match.setDataFromRemote(
					match_id, cUser.getUser_id(), auth_token, ctxCardGraph);
			auxPlayer = currentMatch.getPlayers();
			...
			holes = org.classes.mygolfcard.Hole.getInfoHoles(
					currentMatch.getCourse_id(), auth_token, ctxCardGraph);
		...
		protected void onPreExecute() {
			super.onPreExecute();
			CharSequence title_remote = ctxCardGraph
					.getString(R.string.title_remote_connection);
			CharSequence remote = ctxCardGraph
					.getString(R.string.remote_connection);
			this.dialog = ProgressDialog.show(ctxCardGraph, title_remote,
					remote, true);
		}
